...
#swgsObj = segmentRawData(info=sample.info, raw.data=rawFile, fit.data=corrFile)
#pr = predictRiskFromSegments(swgsObj)

pr = predictRisk(info=sample.info, path=inputPath)
...
recommendations %>%
  ...
  row_spec(nrow(recommendations), bold=T, color=ifelse(recommendations[nrow(recommendations),'Rule'] %in% c(1,4), 'white', 'black'), background = riskCols[recommendations[nrow(recommendations),'Rule',drop=T]]) %>%
  kable_styling("bordered", full_width = F, position = 'left') 
```

### Risk Per Endoscopy

```{r risk_endo, fig.height=4, fig.width=length(levels(pr$segmented$sample.info$Endoscopy))*2}
...
kable(preds[,c('Endoscopy','Risk','img')],col.names=c('Endoscopy','Risk','Absolute Risk'), escape=FALSE) %>% kable_styling(bootstrap_options=c('bordered',full_width=T))
...
```{r risk_sample_plot, fig.height=2+length(unique(pr$segmented$sample.info$GEJ.Distance)*2), fig.width=2+length(levels(pr$segmented$sample.info$Endoscopy))*2}
p = patientRiskTilesPlot(pr)

legend = BarrettsProgressionRisk:::.get.legend(p)
p = p + theme(legend.position = 'none', axis.text.x = element_text(angle=45)) + labs(title='Sample risk by endoscopy', y='Biopsy height', x='Date')
...
```{r sample_info}
table = kable(pr$segmented$sample.info %>% dplyr::select(-matches('Endoscopy')))  %>% kable_styling(bootstrap_options=c('striped','condensed',full_width=T))
...
```{r plot_cn, warning=F, echo=F, fig.width=22, fig.height=length(pr$segmented$seg.plots)*4}
copyNumberMountainPlot(pr, T, F)
```
...
`r kable(segmentedValues(pr)[ sample(1:nrow(segmentedValues(pr)), 10) ,c(1:8)], row.names=F, caption='Example regions of the segmented data.') %>% kable_styling(bootstrap_options=c('striped','condensed',full_width=F, position='left'))`
...
```{r plot_seg, warning=F, echo=F, fig.height=length(pr$segmented$seg.plots)*4, fig.width=22}
plotSegmentData(pr)
```
