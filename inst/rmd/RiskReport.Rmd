...
demoFile = 'example/endoscopy.xlsx'
...
```{r risk_sample, fig.height=length(unique(pr$segmented$sample.info$GEJ.Distance)), fig.width=length(levels(pr$segmented$sample.info$Endoscopy))*2}
printRisk <- function(x,low,high, risk='Unknown') {
  img = switch(risk, 
         'High'='img/Human_body_silhouette-RED.png',
         'Moderate'='img/Human_body_silhouette-YELLOW.png',
...
gridExtra::grid.arrange(gridExtra::arrangeGrob(p,legend,heights=c(8,1)))
```
...
```{r plot_cn, warning=F, echo=F,width=10, fig.height=length(pr$segmented$seg.plots)*4}
...
```{r plot_seg, warning=F, echo=F, fig.height=length(pr$segmented$seg.plots)*4, width=10}
plotSegmentData(pr)
```
